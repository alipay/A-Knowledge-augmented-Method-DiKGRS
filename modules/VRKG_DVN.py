...
args = parse_args()
dvn_hidden_dim = eval(args.dvn_hidden_dim)
gate_log = list()
...
class EmbeddingGate(nn.Module):
    def __init__(self, embedding_dim):
        super(EmbeddingGate, self).__init__()
        self.embedding_dim = embedding_dim
        self.embedding_gate = nn.Linear(embedding_dim * 2, embedding_dim)
    
    def forward(self, input1, input2):
        # print(input1.shape,input2.shape)
        concatenated = torch.squeeze(torch.cat((input1, input2), dim=-1))
        gate = F.sigmoid(self.embedding_gate(concatenated))
        fused_embedding = gate * input1 + (1 - gate) * input2
        #average gate score with epoch
        gate_record = torch.mean(gate, dim = 1).detach().cpu()
        return fused_embedding, gate_record
...
class Recommender(nn.Module):
    def __init__(self, data_config, args_config, graph, adj_mat):
        ...
        self.dvn_input_dim = data_config['dvn_input_dim']

        self.hidden_dim = args_config.dvn_hidden_dim
        ...
        self.dvn_clip = False
        self.abs_dvn_fusion = args_config.abs_dvn_fusion
        self.abs_dvn_up = args_config.abs_dvn_up
        self.abs_dvn_pop = args_config.abs_dvn_pop

        # define DVN model
        if args_config.add_dvn and not args_config.abs_dvn_fusion:
            self.dvn_dropout_rate = args_config.dvn_dropout_rate
            self.dvn_dropout = nn.Dropout(p = self.dvn_dropout_rate )
            if args_config.dataset =='movie' and not args_config.abs_dvn_up:
                dvn_input_dim = 32
            ...
            if args_config.dataset =='movie' and args_config.abs_dvn_pop:
                dvn_input_dim = 30
                self.dvn = nn.Sequential(
                    nn.Linear(dvn_input_dim,self.emb_size//2),
                    nn.ReLU(),
            ...
            if args_config.dataset =='last-fm' or args_config.abs_dvn_up:
                dvn_input_dim = 2
                self.dvn = nn.Sequential(
        ...
        if args_config.add_dvn and args_config.abs_dvn_fusion:
            self.dvn_dropout_rate = args_config.dvn_dropout_rate
            self.dvn_dropout = nn.Dropout(p = self.dvn_dropout_rate )
        ...
        self.pos_gcn = torch.tensor([])
        self.pos_dvn = torch.tensor([])
    ...
    def _init_weight(self):
        ...
        if self.add_dvn and self.dvn_clip:
            initializer(self.dvn.weight)
            nn.init.zeros_(self.dvn.bias)
    ...
    def forward(self, cf_batch):
        ...
        if self.abs_dvn_up:
            dvn_pos_feature = dvn_pos_feature[:,:2]
            dvn_neg_feature = dvn_neg_feature[:,:2]
        if self.abs_dvn_pop:
            dvn_pos_feature = dvn_pos_feature[:,2:]
            dvn_neg_feature = dvn_neg_feature[:,2:]
        ...
        if self.abs_dvn_fusion:
            dvn_pos_feature, dvn_neg_feature = self.dvn_dropout(dvn_pos_feature), self.dvn_dropout(dvn_neg_feature)
            dvn_pos_e, dvn_neg_e = self.dvn(dvn_pos_feature), self.dvn(dvn_neg_feature)
            pos_e, neg_e = self.SimpleFusion(dvn_pos_e,pos_e), self.SimpleFusion(dvn_neg_e,neg_e)
    

        if self.raw_loss:
            return self.create_bpr_loss_raw(u_e, pos_e, neg_e)
        else:
            return self.create_bpr_loss(u_e, pos_e, neg_e, pseudo_label_pos,dvn_pos_weight)
    ...
    def new_generate(self, item_batch):
        ...
        if self.abs_dvn_fusion:
            dvn_item_emb = self.dvn(item_batch_feature)
            i_batch_emb = self.SimpleFusion(dvn_item_emb, i_batch_emb)
    
        return  i_batch_emb, user_gcn_emb
    
    #bpr loss without dvn module
    def create_bpr_loss_raw(self, users, pos_items, neg_items):
    ...
    #bpr loss with pseudo sample weighted loss
    def create_bpr_loss(self, users, pos_items, neg_items,pseudo_label_pos, dvn_pos_weight):
        users = torch.squeeze(users)
        pos_items = torch.squeeze(pos_items)
        neg_items = torch.squeeze(neg_items)
        batch_size = users.shape[0]
        pos_scores = torch.sum(torch.mul(users, pos_items), axis=1)
        neg_scores = torch.sum(torch.mul(users, neg_items), axis=1)
