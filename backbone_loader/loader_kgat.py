...
from backbone_loader.loader_base import DataLoaderBase
...
class DataLoaderKGAT(DataLoaderBase):
    ...
    def construct_data(self, kg_data):
        ...
        self.cf_train_data_flag = (np.array(list(map(lambda d: d + self.n_entities, self.cf_train_data_flag[0]))).astype(np.int32), self.cf_train_data_flag[1].astype(np.int32))
        ...
        self.train_user_dict_flag = {k + self.n_entities: np.array(v).astype(np.int32) for k, v in self.train_user_dict_flag.items()}
